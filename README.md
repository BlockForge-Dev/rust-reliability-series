...
Use one GitHub repo for the entire series.

- Keep `main` as the latest state of the platform.
- Tag every starting point and ending point of an episode.
- Point students to the tag that matches the episode they are watching.
- Keep each episode focused on one reliability boundary, not a grab bag of features.

Suggested tag naming:
...
The roadmap in the PDF turns into four broad phases:

1. Weak system, timeout handling, duplicate execution, and idempotency.
2. Request fingerprinting, response reuse, workers, queue basics, leases, crashed workers, and heartbeats.
3. Retry classification, backoff, jitter, max attempts, dead letters, poison jobs, observability, and attempt histories.
4. Receipts, reconciliation, provider semantics, finality models, polling vs push confirmation, operator review queues, and replay vs repair.

## Teaching promise

Each episode should follow the same loop:

1. Show the current system working.
2. Trigger one failure mode.
3. Explain what is known, unknown, and only inferred.
4. Add one new primitive or boundary.
5. Re-run the same scenario and show the improvement.

## Next steps for the series

- Push this repository to GitHub.
- Tag the current state as the starting point for episode zero.
- Record the happy path first.
- Then film the failure that proves why this design is not enough.
